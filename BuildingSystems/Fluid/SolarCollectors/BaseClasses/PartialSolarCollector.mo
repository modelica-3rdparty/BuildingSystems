within BuildingSystems.Fluid.SolarCollectors.BaseClasses;
partial model PartialSolarCollector "Partial model for solar collectors"
 extends BuildingSystems.Fluid.Interfaces.LumpedVolumeDeclarations;
  ...
  parameter Modelica.Units.SI.HeatCapacity CTot=
    if per.CTyp==BuildingSystems.Fluid.SolarCollectors.Types.HeatCapacity.TotalCapacity then
      per.C
    elseif per.CTyp==BuildingSystems.Fluid.SolarCollectors.Types.HeatCapacity.DryCapacity then
  ...
  parameter BuildingSystems.Fluid.SolarCollectors.Types.NumberSelection nColType=
  BuildingSystems.Fluid.SolarCollectors.Types.NumberSelection.Number
  ...
  parameter Integer nPanels= 0 "Desired number of panels in the simulation"
    annotation(Dialog(group="Area declarations", enable= (nColType == BuildingSystems.Fluid.SolarCollectors.Types.NumberSelection.Number)));

  parameter Modelica.Units.SI.Area totalArea=0
    "Total area of panels in the simulation" annotation (Dialog(group=
          "Area declarations", enable=(nColType == BuildingSystems.Fluid.SolarCollectors.Types.NumberSelection.Area)));

  parameter BuildingSystems.Fluid.SolarCollectors.Types.SystemConfiguration sysConfig=
  BuildingSystems.Fluid.SolarCollectors.Types.SystemConfiguration.Series
  ...
  parameter Integer nPanelsPar=0 "Number of array panels in parallel"
    annotation(Dialog(group="Configuration declarations", enable= (sysConfig == BuildingSystems.Fluid.SolarCollectors.Types.SystemConfiguration.Array)));
  ...
  BuildingSystems.BoundaryConditions.SolarIrradiation.DiffusePerez HDifTilIso(
  ...
  BuildingSystems.BoundaryConditions.SolarIrradiation.DirectTiltedSurface HDirTil(
    final til=til,
  ...
  replaceable parameter BuildingSystems.Fluid.SolarCollectors.Data.GenericASHRAE93 per
    constrainedby BuildingSystems.Fluid.SolarCollectors.Data.BaseClasses.Generic
    "Performance data"
...
protected
  ...
  parameter Real nPanels_internal=
    if nColType == BuildingSystems.Fluid.SolarCollectors.Types.NumberSelection.Number then
      nPanels
    else
      totalArea/per.A "Number of panels used in the simulation";
  parameter Real nPanelsSer_internal=
    if sysConfig == BuildingSystems.Fluid.SolarCollectors.Types.SystemConfiguration.Series then
      nPanels_internal
    else if sysConfig == BuildingSystems.Fluid.SolarCollectors.Types.SystemConfiguration.Parallel then
      1
    else
      nPanels_internal/nPanelsPar_internal "Number of panels in series";
  parameter Real nPanelsPar_internal=
    if sysConfig == BuildingSystems.Fluid.SolarCollectors.Types.SystemConfiguration.Parallel then
      nPanels_internal
    else if sysConfig == BuildingSystems.Fluid.SolarCollectors.Types.SystemConfiguration.Series then
      1
    else
...
e.g., if <code>sysConfig == BuildingSystems.Fluid.SolarCollectors.Types.SystemConfiguration.Parallel</code>.<br/>
